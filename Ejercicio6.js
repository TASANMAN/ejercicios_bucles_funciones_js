...
for (let i = 0; i < starWarsMovies.length; i++) {
  ...
  // Determinamos la década de lanzamiento
  const decade = Math.floor(movie.releaseYear / 10) * 10

  // Si la década aún no existe en el objeto, la inicializamos con un array vacío
  if (!moviesByDecade[decade]) {
    moviesByDecade[decade] = []
  }

  // Añadimos la película al array correspondiente en el objeto
